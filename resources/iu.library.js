...
window.iu = {
    '__version': {
        ...
        "__updateLog": [
            ...
            {
                "version": "1.2.9.211117(1019) to 1.2.10.211117(1046)",
                "info": "1.更新了版本号"
            }
        ]
    },
    'time': {
        ...
        'nowTimestamp': function (_length) {
            //取当前时间戳
            let time = new Date().getTime();
            return parseInt(_length) === 10 ? parseInt((time / 1000).toString()) : time;
        },
        'getTimes': function (_timeStr) {
            //取时间
            return new Date(_timeStr.replace(/-/g, '/'));
        },
        'getTimestamp': function (_timeStr,_length) {
            //取时间戳
            let time = new Date(_timeStr.replace(/-/g, '/')).getTime();
            return parseInt(_length) === 10 ? parseInt((time / 1000).toString()) : time;
        }
    },
    'http': {
        'res': function (method, url, is_ajax, data, header,callback) {
            //XHR
            // header 数据结构
            // [
            //     {
    ...
    'html': {
        ...
        'getType': function (_function){
            //取变量类型
            return typeof _function;
        },
        'getScreenOrientation': function(type){
            //计算屏幕方向
            let _W = document.documentElement.clientWidth || document.body.clientWidth;
            let _H = document.documentElement.clientHeight || document.body.clientHeight;
            if(type === "boolean"){
                return _W > _H;// true 为横向，false 为纵向
            }
            return _W > _H ? "X" : "Y";// X 为横向，Y 为纵向
        }
    },
    'systemInfo': {
        'language': ((navigator.language) ? navigator.language : navigator.userLanguage).replace('-','_')
    },
    'string': {
        'left': function (_str,_length){
            //取字符串左边
            return _str.length <= _length ? _str : _str.substring(0, _length);
        },
        'right': function (_str,_length){
            //取字符串右边
            return _str.length <= _length ? _str : _str.substring(_str.length - _length, _str.length);
        },
        'center': function(_str,_start,_end){
            //取字符串中间
            _str.substring(_start,Math.min(_end,_str.length));
        }
    },
    'QRCode': {
        'generate': function(text,size,img,img_size,background_color,foreground_color,pdground_color){
            foreground_color = foreground_color ? foreground_color : "#000000";
            if(img){
                return new AraleQRCode({
                    "render": "svg", // 生成的类型 'svg' or 'table'
                    "text": text ? text : "null", // 需要生成二维码的链接
                    "size": size ? parseInt(size) : 200, // 生成二维码大小
                    "image": img,
                    "imageSize": img_size ? img_size : 30,
                    "background": background_color ? background_color : "#FFFFFF",
                    "foreground": foreground_color,
                    "pdground": pdground_color ? pdground_color : foreground_color
                });
            }
            return new AraleQRCode({
                "render": "svg", // 生成的类型 'svg' or 'table'
        ...
        'addToHTML': function (element,text,size,ID){
            let e = size ? iu.QRCode.generate(text,size) : iu.QRCode.generate(text);
            if(element){
                if(ID){
                    e.setAttribute("id",ID);
                }
                element.appendChild(e);
                return "已将二维码添加到页面" + (size ? "" : "(您未设置生成大小，默认宽高200px)");
            }else {
                return "指定的赋值元素不存在";
            }
        }
